...
const ProfileBox = ({ result = {}, children = null }) => {
  const isClient = window.location.href.includes("customer");
  const imageSource = useMemo(() => {
    const imgSrc = window.location.href.includes("customer")
      ? result.customer_file
      : result.trainer_file;
    return imgSrc === "null" ? imageDefault : imgSrc;
  }, [result.customer_file, result.trainer_file]);
  return (
    <div
      style={{
        display: "flex",
      ...
      <div className="box-element-1">
        <div>
          ...
          <div className="profileName">
            {isClient ? result.customer_name : result.name}
          </div>
          <div style={{ fontSize: "12px" }}>
            {isClient ? "Klient" : "Trener"}
          </div>
          <div style={{ fontSize: "12px" }}>
            Wiek: {isClient ? result.customer_age : result.age}
            lat
          </div>
        </div>
        {children}
        <div className="contact-info">
          ...
          {isClient ? result.customer_phone : result.phone}
        </div>
        <div className="contact-info">
          <div className="contact-icon-wrapper">
            <svg
          ...
          {isClient ? result.customer_email : result.email}
        </div>
      </div>
      {isClient ? (
        <div className="box-element-2">
          <form className="customers-form-profile">
              ...
              <div className="form-group">
                <label className="text-label">
                  ...
                  <input
                    ...
                    required
                    className="form-control"
                    placeholder={result?.harwardResult}
                  />
                </label>
              </div>
            </div>
              ...
              <div className="form-group">
                <label className="text-label">
                  ...
                  <input
                    ...
                    required
                    readOnly
                    placeholder={result?.strengthTest}
                  />
                </label>
              </div>
            </div>
              ...
              <div className="form-group">
                <label className="text-label">
                  ...
                  >
                    ...
                    <option value="Total Body Workout">
                      Total Body Workout
                    </option>
                  </select>
                </label>
